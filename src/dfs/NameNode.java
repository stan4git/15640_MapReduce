...
public class NameNode extends UnicastRemoteObject implements NameNodeInterface {
	...
	public ConcurrentHashMap<String, Hashtable<Integer, HashSet<String>>> generateChunkDistributionList(
		...
		for (Entry<String, Hashtable<Integer, HashSet<String>>> fileTuple : failureList
				.entrySet()) {
			// get filename
	 ...
	 * @throws RemoteException
	 */
	public String pickMostAvailableSlotDataNode(HashSet<String> excludeList)
			throws RemoteException {
	...
	// public ConcurrentHashMap<String, Hashtable<Integer, HashSet<String>>>
	// getFileDistributionTable(
	// String filename) {
	// ConcurrentHashMap<String, Hashtable<Integer, HashSet<String>>> returnList
	...
	@Override
	public void removeChunkFromFileDistributionTable(String filename,
			int chunkNum, String dataNodeIP) {
		this.fileDistributionTable.get(filename).get(chunkNum)
				.remove(dataNodeIP);
		System.out.println("Chunk" + chunkNum + " of file \"" + filename
				+ "\" on " + dataNodeIP
				+ " has been removed from file distribution table...");
		return;
	}

	// public void updateFileDistributionTable(
	// ConcurrentHashMap<String, Hashtable<Integer, HashSet<String>>>
	// tableToBeUpdated) {
	// for (Entry<String, Hashtable<Integer, HashSet<String>>> fileTuple :
	// tableToBeUpdated.entrySet()) {
	// String filename = fileTuple.getKey();
	...
	// } else {
	// for (Entry<Integer, HashSet<String>> chunkTuple :
	// fileTuple.getValue().entrySet()) {
	// int chunkNum = chunkTuple.getKey();
